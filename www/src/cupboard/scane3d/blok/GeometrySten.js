export function GeometrySten() {
    ...
    this._textureWidth = 50; // ширина текстуры / сегмента
    this._textureHeight = 50; // высота текстуры / сегмента
    this._depth = 1; // глубина боксов
    this._z = 0;
    ...
    this.startArrRect = function () {
        ...
        if (this._isInside) {
            // если нужно отображать коробки которые в нутри большой коробки
            // заполняем
            ...
            // показывать большую коробку исключая маленькие коробки
    ...
    this.fillPointExcludeBox = function () {
        ...
        for (var i = 0; i < this._arrBox.length; i++) {
            // берем позиции коробки если они в нутри большой коробки
            // if(this._arrBox[i].boxGeom!=undefined)objDin=this._arrBox[i].boxGeom;
            // else
            objDin = this._arrBox[i];
            //if(this._arrBox[i].boxGeom!=null){
            if (
                this._rect.x < objDin.x + objDin.width &&
        ...
        for (
            var i = this._rect.x + this._textureWidth;
            i < this._rect.x + this._rect.width;
            i += this._textureWidth
        ) {
            setkaBigX[offX++] = i;
        }
        for (
            var i = this._rect.y + this._textureHeight;
            i < this._rect.y + this._rect.height;
            i += this._textureHeight
        ) {
            setkaBigY[offY++] = i;
        }
        ...
        // сортируем
        setkaBigY.sort(function (a, b) {
            return a - b;
        });
        setkaBigX.sort(function (a, b) {
            return a - b;
        });
        ...
        if (freeBigBox) {
            // значит в большой коробки других коробок нет
            this.fillPointFromBox(
                setkaBigX[0],
                setkaBigY[0],
                Math.abs(setkaBigX[setkaBigX.length - 1] - setkaBigX[0]),
                Math.abs(setkaBigY[setkaBigY.length - 1] - setkaBigY[0]),
            );
        } else {
    ...
    this.fillPointFromBox = function (_x, _y, _w, _h) {
        ...
        // сортируем
        setkaY.sort(function (a, b) {
            return a - b;
        });
        setkaX.sort(function (a, b) {
            return a - b;
        });
        ...
        for (var i = 0; i < setkaX.length; i++) {
            if (
                setkaX[i] == setkaX[i + 1] ||
                setkaX[i + 1] > this._rect.x + this._rect.width
            ) {
                setkaX.splice(i + 1, 1);
                i--;
            }
        ...
        for (var i = 0; i < setkaY.length; i++) {
            if (
                setkaY[i] == setkaY[i + 1] ||
                setkaY[i + 1] > this._rect.y + this._rect.height
            ) {
                setkaY.splice(i + 1, 1);
                i--;
            }
    ...
    this.inBox = function (x, y, x1, y1) {
        for (var i = 0; i < this._arrBox.length; i++) {
            ...
            if (
                x >= objDin.x &&
                x1 <= objDin.x + objDin.width &&
                y >= objDin.y &&
                y1 <= objDin.y + objDin.height
            ) {
                return true;
            }
            // }
    ...
    this.fillPointOptimiz = function (x, y) {
        ...
        var pow = 0,
            temp = 0,
            ind = 0; // pow -  максимальная мощь ректа, ind - индекс максимального ректа в масиве
        for (var j = x + 1; lasy < setkaBigY.length; j++) {
            if (
                Math.abs(
                    ((setkaBigY[lasy] - this._rect.y) / this._textureHeight) %
                        1,
                ) == 0 &&
                lasy > y
            ) {
                // если вышли ниже сегмента текстуры
                break;
            }
            // если столкнулись |или дошли до конца по x|или до сегмента
            if (
                arrBool[lasy][j] ||
                j == setkaBigX.length - 1 ||
                Math.abs(
                    ((setkaBigX[j] - this._rect.x) / this._textureWidth) % 1,
                ) == 0
            ) {
        ...
        this.fillPointFromBox(
            setkaBigX[x],
            setkaBigY[y],
            Math.abs(setkaBigX[arrXY[ind][0]] - setkaBigX[x]),
            Math.abs(setkaBigY[arrXY[ind][1]] - setkaBigY[y]),
        );
        this.setBoolRect(x, y, arrXY[ind][0], arrXY[ind][1]);
    };

    // задаем рект в масив arrBool
    this.setBoolRect = function (x, y, x1, y1) {
        // x = y; y = x
    ...
    this.getBoolRectPow = function (x, y, x1, y1) {
        // i = y; j = x
        var pow = 0;
        for (var yy = y; yy < arrBool.length && yy < y1; yy++) {
            for (var xx = x; xx < arrBool[yy].length && xx < x1; xx++) {
    ...
    this.fillPointSetka = function () {
        ...
        for (var i = 0; i < setkaY.length - 1; i++) {
            for (var j = 0; j < setkaX.length - 1; j++) {
                ...
                px =
                    Math.abs(((x - this._rect.x) / this._textureWidth) % 1) ||
                    0;
                py =
                    Math.abs(((y - this._rect.y) / this._textureHeight) % 1) ||
                    0;
                px1 =
                    Math.abs(((x1 - this._rect.x) / this._textureWidth) % 1) ||
                    1;
                py1 =
                    Math.abs(((y1 - this._rect.y) / this._textureHeight) % 1) ||
                    1;
    ...
    this.createBuffer = function () {
        ...
        for (var i = 0; i < arrPosition.length; i++) {
            this.attributes.position.array[i] =
                Math.round(arrPosition[i] * 100) / 100; // = arrPosition[i];
        }
